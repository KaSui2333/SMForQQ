...
mouseButton = ttk.Button(mainForm, text='获取鼠标位置', width=40, command=onClickMouseButton)
...
def onClickStartButton():
    ...
    if nameContent == '群发名单:(此行无需删除，用于试错)' or nameContent == '':
        msg.showerror('错误', '请先输入名单')
        return

    msgContent = msgText.get(0.0, tk.END).rstrip()

    if msgContent == '群发内容(删除此行)' or msgContent == '':
        msg.showerror('错误', '请先输入群发的内容')
        return

    # 开始群发
    if not msg.askokcancel('提示', '开始，完成前请勿移动鼠标'):
        return
